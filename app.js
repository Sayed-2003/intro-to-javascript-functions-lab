...
function basicCalculator(num1, num2, str) {
    if (str === 'add') {
        ...
        throw new Error("enter a valid number or string representing an operation ('add', 'subtract', 'multiply', 'divide'). ")
    }
...
function createUsername(fName, lName) {
    let combinedUser = fName.slice(0, 3) + lName.slice(0, 3);
    let length = fName.length + lName.length
    return `${combinedUser}${length}`
